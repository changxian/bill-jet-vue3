...
<script lang="ts" setup>
  ...
  import { loadTreeData, saveOrUpdateDict, uniqueDictCheck } from '../category.api';
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    //父级节点树信息
    treeData.value = await loadTreeData({ async: false, parentId: '' });
    updateSchema({
      field: 'parentId',
  ...
  async function handleSubmit() {
    ...
    if (name && name.length > 0) {
      uniqueDictCheck({ name: name }).then((res) => {
        debugger;
        if (res != null && res.id) {
          createMessage.warning('当前分类名称已存在，请重新输入！');
        } else {
          submit();
        }
      });
