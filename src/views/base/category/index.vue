<template>
  <div>
    ...
    >
      <!--插槽:table标题-->
      <template #tableTitle>
        <a-button type="primary" v-auth="'bill:jxc_goods_category:addTree'" preIcon="ant-design:plus-outlined" @click="handleCreate"> 新增</a-button>
        ...
        <a-dropdown v-if="selectedRowKeys.length > 0">
          ...
          <a-button v-auth="'bill:jxc_goods_category:deleteTreeBatch'"
            >批量操作
    ...
    <!--字典弹窗-->
    <GoodsCategoryModal @register="registerModal" @success="handleSuccess" />
  </div>
</template>

<script lang="ts" name="goods-category" setup>
  //ts语法
  import { ref, computed, unref, toRaw, nextTick } from 'vue';
  import { BasicTable, useTable, TableAction } from '/src/components/Table';
  import GoodsCategoryModal from './components/GoodsCategoryModal.vue';
  ...
  const { prefixCls, onExportXls, onImportXls, tableContext } = useListPage({
    designScope: 'goods-category-template',
    tableProps: {
      title: '商品分类',
      api: list,
      columns: columns,
    ...
    if (unref(expandedRowKeys).length > 0) {
      ...
      if (res.success && res.result.records.length > 0) {
        ...
        for (let item of records) {
          ...
          if (unref(expandedRowKeys).includes(parentId)) {
            let mapList = listMap.get(parentId);
            if (mapList == null) {
              mapList = [];
            }
            mapList.push(item);
            listMap.set(parentId, mapList);
          }
        }
    ...
    return [
      {
        ...
        auth: 'bill:jxc_goods_category:editTree',
      },
      {
        label: '删除',
        popConfirm: {
        ...
        auth: 'bill:jxc_goods_category:deleteTree',
      },
      {
        label: '添加下级',
        onClick: handleAddSub.bind(null, { parentId: record.id }),
        auth: 'bill:jxc_goods_category:addTree',
      },
    ];
  }
</script>
