<template>
  <BasicDrawer v-bind="$attrs" @register="registerDrawer" width="650px" destroyOnClose showFooter>
    ...
    <PackDataRuleDrawer @register="registerDrawer1" />
  </BasicDrawer>
</template>
<script lang="ts" setup>
  ...
  import PackDataRuleDrawer from './PackDataRuleDrawer.vue';
  import { queryTreeListForRole, queryRolePermission, saveRolePermission } from '../SysPack.api';
  ...
  const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
    ...
    // update-begin--author:liaozhiyang---date:20240228---for：【QQYUN-8355】用户权限配置的菜单翻译
    treeData.value = translateTitle(roleResult.treeList);
    // update-end--author:liaozhiyang---date:20240228---for：【QQYUN-8355】用户权限配置的菜单翻译
    allTreeKeys.value = roleResult.ids;
    // update-begin--author:liaozhiyang---date:20240531---for：【TV360X-590】用户授权弹窗操作缓存
    ...
    // update-end--author:liaozhiyang---date:20240531---for：【TV360X-590】用户授权弹窗操作缓存
    //初始化用户菜单数据
    const permResult = await queryRolePermission({ packId: unref(packId) });
  ...
  * 翻译菜单名称
  ...
  function onTreeNodeSelect(key) {
    ...
    openDataRuleDrawer(true, { functionId: unref(selectedKeys)[0], packId: unref(packId) });
  }
  /**
   * 数据重置
   */
  ...
  async function handleSubmit(exit) {
    ...
    //update-begin-author:taoyan date:2023-2-11 for: issues/352 VUE用户授权重复保存
  ...
  //update-end-author:taoyan date:2023-2-11 for: issues/352 VUE用户授权重复保存
