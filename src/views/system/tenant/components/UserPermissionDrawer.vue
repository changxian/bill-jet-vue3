<template>
  <BasicDrawer v-bind="$attrs" @register="registerDrawer" width="650px" destroyOnClose showFooter>
    ...
    <UserDataRuleDrawer @register="registerDrawer1" />
  </BasicDrawer>
</template>
<script lang="ts" setup>
  ...
  import UserDataRuleDrawer from './UserDataRuleDrawer.vue';
  import { queryTreeListForRole, queryRolePermission, saveRolePermission } from '../user.api';
  ...
  const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
    ...
    //初始化用户菜单数据
    const permResult = await queryRolePermission({ userId: unref(userId) });
  ...
  * 翻译菜单名称
  ...
  function onTreeNodeSelect(key) {
    ...
    openDataRuleDrawer(true, { functionId: unref(selectedKeys)[0], userId: unref(userId) });
  }
  /**
   * 数据重置
   */
