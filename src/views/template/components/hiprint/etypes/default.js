...
function roil(obj) {
  ...
  for (const a in obj) {
    if (1 === obj[a].n) {
      ...
      const array = obj[a].arr;
      for (let i = 1; i <= array.length; i++) {
        const name = i === 1 ? a : a + i;
        arr.push({
          field: name,
          text: obj[a].arr[i - 1],
        });
...
const fields = [
  ...roil(fieldConfig),
  // 从配置中读取，调roil函数 构建也如下面的结构（此结构在设计模板中，table列的 field 下拉选项
  // { text: '序号', field: 'seq' },
  // { text: '行号', field: 'seq2' },
  // { text: 'NO', field: 'seq3' },
  ...
  // { text: '长度(m)', field: 'length2' },
  //
  // { text: '宽(m)', field: 'wide' },
  // { text: '宽度(m)', field: 'wide2' },
  //
  // { text: '高(m)', field: 'high' },
  ...
  // { text: '金额（RMB）', field: 'amount5' },
  //
  // { text: '备注', field: 'remark' },
  // { text: '備注', field: 'remark2' },
  //
  // { text: 'Description', field: 'description' },
  //
  // { text: '产商', field: 'firm' },
  // { text: '厂商', field: 'firm2' },
  // { text: '生产产商', field: 'firm4' },
