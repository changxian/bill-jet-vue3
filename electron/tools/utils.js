...
function initServeEvent(server) {
  ...
  server.on('connect', async (socket) => {
    ...
    // 通知渲染进程已连接
    MAIN_WINDOW.webContents.send('serverConnection', server.engine.clientsCount);
    ...
    // 向 client 发送打印机列表
    socket.emit('printerList', await MAIN_WINDOW.webContents.getPrintersAsync());
    ...
    socket.on('ippPrint', (options) => {
      ...
      try {
        ...
        printer.execute(action, msg, (err, res) => {
          socket.emit('ippPrinterCallback', err ? { type: err.name, msg: err.message } : null, res);
        });
      } catch (error) {
    ...
    socket.on('ippRequest', (options) => {
      ...
      try {
        ...
        ipp.request(url, _data, (err, res) => {
          socket.emit('ippRequestCallback', err ? { type: err.name, msg: err.message } : null, res);
        });
      } catch (error) {
...
function initClientEvent() {
  ...
  client.on('connect', async () => {
    ...
    // 向 中转服务 发送打印机列表
    client.emit('printerList', await MAIN_WINDOW.webContents.getPrintersAsync());
  ...
  client.on('ippPrint', (options) => {
    ...
    try {
       ...
       */
      printer.execute(action, msg, (err, res) => {
        client.emit('ippPrinterCallback', err ? { type: err.name, msg: err.message, replyId } : { replyId }, res);
      });
    } catch (error) {
  ...
  client.on('ippRequest', (options) => {
    ...
    try {
      ...
      let _data = ipp.serialize(data);
      ipp.request(url, _data, (err, res) => {
        client.emit('ippRequestCallback', err ? { type: err.name, msg: err.message, replyId } : { replyId }, res);
      });
    } catch (error) {
